...
console.log("firebaseUser",firebaseUser);
          ...
          try {
            ...
            try {
              ...
              if (response.success && response.data) {
                ...
                const cachedUser = currentCachedData ? JSON.parse(currentCachedData) : null;
                
                // Important: Only update subscription if it's explicitly provided by backend
                // or if we don't have it cached already
                const subscription = response.data.subscription !== undefined 
                ...
                localStorage.setItem('cached_user_data', JSON.stringify(basicUser));
              }
            } catch (error) {
              ...
              localStorage.setItem('cached_user_data', JSON.stringify(fallbackUser));
            }
          } catch (error) {
            console.error('Error during backend login:', error);
            toast.error('Failed to connect to server. Using local authentication only.');
            ...
            localStorage.setItem('cached_user_data', JSON.stringify(offlineUser));
          }
        } catch (error) {
  ...
  return (
    <AuthContext.Provider value={{ user, loading, firebaseUser, refreshToken, refreshUserData }}>
      {children}
