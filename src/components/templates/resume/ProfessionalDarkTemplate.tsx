  ...
  return (
    <div className="w-full min-h-screen flex font-sans text-xs">
      ...
      <div className="w-1/3 bg-slate-800 text-white p-2 flex flex-col max-h-screen overflow-y-auto flex-grow">
        ...
        {/* CV title */}
        <h2 data-field="name" {...(editMode ? editableProps : {})} className="text-sm font-semibold text-center text-white mb-3">
          ...
          <div className="space-y-0.5">
            {data.personalInfo.email && (
              <p className="flex items-center gap-1 text-xs">
                ...
                <span data-field="email" {...(editMode ? editableProps : {})} className="break-words w-24">
                  {data.personalInfo.email}
                </span>
              </p>
            )}
            {data.personalInfo.phone && (
              <p className="flex items-center gap-1 text-xs">
                ...
                <span data-field="phone" {...(editMode ? editableProps : {})} className="break-words w-full">
                  {data.personalInfo.phone}
                </span>
              </p>
            )}
            {data.personalInfo.location && (
              <p className="flex items-center gap-1 text-xs">
                <span className="inline-block w-4">🏠</span>
                <span data-field="location" {...(editMode ? editableProps : {})} className="break-words w-full">
                  {data.personalInfo.location}
                </span>
              </p>
            )}
            {data.personalInfo.linkedin && (
              <p className="flex items-center gap-1 text-xs">
                <span className="inline-block w-4">🔗</span>
                <span data-field="linkedin" {...(editMode ? editableProps : {})} className="break-words w-full">
                  {data.personalInfo.linkedin}
                </span>
              </p>
            )}
          ...
          <div className="mb-4">
            ...
            {data.skills.filter(skill => skill.skills).map((skill, index) => (
              <div key={index} className="mb-2">
                <h4 className="font-semibold text-xs text-white">{skill.category}</h4>
        ...
        {data.languages?.length > 0 && (
          <div className="mb-4">
            <h3 className="text-sm font-semibold border-b border-slate-600 pb-1 mb-2">Languages</h3>
        ...
        {data.certifications?.some(cert => cert.name) && (
          <div className="mb-4">
            <h3 className="text-sm font-semibold border-b border-slate-600 pb-1 mb-2 text-white">Certifications</h3>
        ...
        {data.interests?.length > 0 && (
          <div className="mb-4">
            <h3 className="text-sm font-semibold border-b border-slate-600 pb-1 mb-2">Interests</h3>
            <ul className="list-none space-y-0.5 text-xs">
    ...
    >
      {data.summary}
        ...
        {data.experience?.some(exp => exp.company || exp.position) && (
          <div className="mb-6">
            <h2 className="text-base font-semibold mb-3">Professional Experience</h2>
              ...
              <div key={index} className="mb-4">
                <div className="flex justify-between items-start mb-0.5 text-xs">
                  <div>
                    ...
                    {project.technologies && <p className="text-[10px]">{project.technologies}</p>}
                  </div>
                  <span className="text-[10px]">{project.date || ''}</span>
                </div>
                {project.description && <p className="text-xs mt-1 break-words">{project.description}</p>}
                {project.link && <div className="text-[10px] mt-0.5 inline-block">{project.link}</div>}
              </div>
            ))}
          </div>
        )}
        ...
        {data.education?.some(edu => edu.institution || edu.degree) && (
          <div className="mb-6">
            <h2 className="text-base font-semibold mb-3">Education</h2>
