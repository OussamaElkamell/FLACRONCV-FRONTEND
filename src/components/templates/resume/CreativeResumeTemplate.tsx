    ...
    <div className="font-sans p-2 mx-auto">
      ...
      {data.summary && (
        <div className="mb-2 bg-purple-50 p-6 rounded-lg border-l-4 border-purple-500">
          <h2 className="text-md font-bold text-purple-700 mb-3">About Me</h2>
          <p 
                ...
                exp.company || exp.position ? (
                  <div key={index} className="mb-6 relative pl-6 before:absolute before:left-0 before:top-2 before:w-3 before:h-3 before:bg-purple-400 before:rounded-full">
                    ...
                    <div className="flex justify-between">
                      <h4 className="text-purple-600 font-normal text-xs">{exp.company || 'Company'}</h4>
                      <span className="text-xs text-gray-500 italic">{exp.date || 'Date'}</span>
                    </div>
                    {exp.description && <p className="text-xs mt-1 break-words">{exp.description}</p>}
                  </div>
                ) : null
              ))}
            </div>
          ...
          {data.projects?.length > 0 && (
            <div className="mb-2">
              ...
              {data.projects.map((project, index) => (
                <div key={index} className="mb-6 relative pl-6 before:absolute before:left-0 before:top-2 before:w-3 before:h-3 before:bg-purple-400 before:rounded-full">
                  <h3 className="text-md font-bold">{project.name || 'Project Name'}</h3>
                  <div className="flex justify-between">
                    {project.technologies && (
                      <h5 className="text-purple-600 font-normal text-xs">{project.technologies}</h5>
                    )}
                    {project.date && (
            ...
            <div className="mb-2">
              ...
              {data.education.map((edu, index) => (
                edu.institution || edu.degree ? (
                  <div key={index} className="mb-6 relative pl-6 before:absolute before:left-0 before:top-2 before:w-3 before:h-3 before:bg-purple-400 before:rounded-full">
                    <h3 className="text-xs font-bold">{edu.degree || 'Degree'}</h3>
                    <div className="flex justify-between">
                      <h5 className="text-purple-600 font-normal text-xs">{edu.institution || 'Institution'}</h5>
                      <span className="text-sm text-gray-500 italic">{edu.date || 'Date'}</span>
                    </div>
                    {edu.description && <p className="text-xs mt-1 break-words">{edu.description}</p>}
                  </div>
                ) : null
              ))}
            </div>
          ...
          {data.certifications?.length > 0 && (
            <div className="mb-2">
              ...
              {data.certifications.map((cert, index) => (
                <div key={index} className="mb-2 relative pl-6 before:absolute before:left-0 before:top-2 before:w-3 before:h-3 before:bg-purple-400 before:rounded-full">
                  <div className="flex justify-between">
