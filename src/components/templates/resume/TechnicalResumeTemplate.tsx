            ...
            <div className="mb-6 bg-indigo-50 p-4 rounded-lg">
              ...
              {data.skills.filter(skillCategory => skillCategory.skills).map((skillCategory, categoryIndex) => (
                <div key={categoryIndex} className="mb-3 last:mb-0">
                  <h3 className="text-sm font-bold text-indigo-700 mb-2">{skillCategory.category}</h3>
            ...
            <div className="mb-6">
              ...
              {data.projects.filter(project => project.name).map((project, index) => (
                <div key={index} className="mb-4 last:mb-0">
                  ...
                  {project.technologies && (
                    <div className="mb-1 flex flex-wrap gap-1">
                      {project.technologies.split(',').map((tech, i) => (
                        <span key={i} className="bg-indigo-100 text-indigo-700 px-1.5 py-0.5 text-xs rounded">
                          {tech.trim()}
                        </span>
                      ))}
                    </div>
                  ...
                  {project.link && (
                    <a href={project.link} target="_blank" rel="noreferrer" className="text-xs text-indigo-600 hover:underline">
                      {project.link}
                    </a>
                  )}
                </div>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <div className="flex">
                      {[...Array(5)].map((_, i) => (
                        <div 
                          key={i} 
                          className={`w-2 h-2 mx-0.5 rounded-full ${i < lang.proficiency ? 'bg-indigo-600' : 'bg-gray-300'}`}
                        />
                      ))}
                    </div>
                  </div>
