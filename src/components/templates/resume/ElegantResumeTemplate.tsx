              ...
              {data.experience.filter(exp => exp.company || exp.position).map((exp, index) => (
                <div key={index} className="mb-6">
                  <div className="flex justify-between items-baseline mb-1">
                    <h3 className="font-semibold text-amber-900">{exp.position || ''}</h3>
          ...
          {data.education && data.education.length > 0 && data.education.some(edu => edu.institution || edu.degree) && (
            <div className="mb-8">
              ...
              {data.education.filter(edu => edu.institution || edu.degree).map((edu, index) => (
                <div key={index} className="mb-6">
                  <div className="flex justify-between items-baseline mb-1">
                    <h3 className="font-semibold text-amber-900">{edu.degree || ''}</h3>
                    <span className="text-sm text-gray-600 italic">{edu.date || ''}</span>
                  </div>
              ...
              {data.projects.filter(project => project.name).map((project, index) => (
                <div key={index} className="mb-6">
                  <div className="flex justify-between items-baseline mb-1">
                    ...
                    <span className="text-sm text-gray-600 italic">{project.date || ''}</span>
                  </div>
                  ...
                  {project.link && (
                    <a href={project.link} target="_blank" rel="noreferrer" className="text-sm text-amber-700 hover:underline">
                      {project.link}
                    </a>
                  )}
                </div>
            ...
            <div className="bg-amber-50 p-4 rounded-lg">
              ...
              {data.skills.filter(skillCategory => skillCategory.skills).map((skillCategory, categoryIndex) => (
                <div key={categoryIndex} className="mb-3">
                  <h3 className="font-medium text-amber-800 mb-2">{skillCategory.category}</h3>
                  <div className="text-sm">
                    {skillCategory.skills.split(',').map((skill, index, array) => (
                      <span key={index}>
                        {skill.trim()}
              ...
              {data.certifications.filter(cert => cert.name).map((cert, index) => (
                <div key={index} className="mb-2">
                  <div className="text-sm font-medium text-amber-900">
                    {cert.title && typeof cert.title === 'string' ? `${cert.title}` : ''}
                  </div>
                ...
                <div key={index} className="mb-2 flex justify-between items-center">
                  ...
                  <div className="flex">
                    {[...Array(5)].map((_, i) => (
                      <div 
                        key={i} 
                        className={`w-2 h-2 mx-0.5 rounded-full ${i < lang.proficiency ? 'bg-amber-600' : 'bg-amber-200'}`}
                      />
                    ))}
                  </div>
                </div>
